...
import { Events, IOrder, IProduct, FormOrderErrors, IContacts, FormConactsErrors} from '../../types/index';
...
export class ProductsData extends Model<ProductsData> {
	...
	setPreview(item: IProduct) {
		...
		if (item.inBasket === true) {} else item.inBasket = false;
	...
	clearBasket(){
		let _filter = this.catalog.filter((item) => item.inBasket === true);
	...
	getTotalPrice() {
		let _basket = this.catalog.filter((item) => item.inBasket === true);
		let total = 0;
	...
	setContactsToOrder(){
		let _phone = this.contacts.phone;
		let _email = this.contacts.email;
